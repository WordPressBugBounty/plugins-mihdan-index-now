...
class Injector
{
    ...
    private function provisionInstance($className, $normalizedClass, array $args)
    {
        try {
            ...
            if (!$ctor) {
                ...
                $obj = $this->reflector->getClass($className)->newInstanceArgs($this->provisionFuncArgs($ctor, $aggregatedArgs, $ctorParams, $className));
            } else {
                $obj = $this->instantiateWithoutCtorParams($className);
            }
            return $obj;
    ...
    private function buildArgFromTypeHint(\ReflectionFunctionAbstract $reflFunc, \ReflectionParameter $reflParam)
    {
        ...
        if (!$typeHint) {
            ...
            if (\array_key_exists($normalizedName, $this->aliases) || \array_key_exists($normalizedName, $this->delegates) || \array_key_exists($normalizedName, $this->shares) || \class_exists($typeHint)) {
